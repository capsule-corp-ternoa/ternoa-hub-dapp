...
>(({ currentNetworkName, onClick, isLoading, className = "" }, ref) => {
  const onClickEnabled = !isLoading;

  return (
    <div
      className={`h-s40 bg-gray-500 min-w-[auto] md:min-w-[140px] rounded-[7px] font-AirbnbCerealBold inline-flex items-center justify-between md:pl-[14px] md:pr-[19px] pl-s8 pr-s8 border-gray-800 border-solid border ${
    ...
    >
      <Text
        ...
        className="pr-s8"
      />
      <Icon name="CaretDown" size={16} color="black" />
    </div>
  );
});
